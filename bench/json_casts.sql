...
-- precision of a float, which doubles the size of the json and narrows the
-- gap between json and jsonb.
--
-- Usage:
...
ALTER TABLE bench_casts ADD COLUMN j json;
ALTER TABLE bench_casts ADD COLUMN v vector(:dim);
UPDATE bench_casts SET j = b::text::json, v = b::text::vector(:dim);
...
FROM (VALUES
	...
	('json -> vector', 'SELECT count(j::vector(%1$s)) FROM bench_casts'),
	...
	('json -> halfvec', 'SELECT count(j::halfvec(%1$s)) FROM bench_casts'),
...
FROM (VALUES
	...
	('insert from json', 'INSERT INTO bench_casts_sink SELECT j::vector(%1$s) FROM bench_casts; TRUNCATE bench_casts_sink')
) AS t(ingest, query);
