...
# return up to hnsw.ef_search tuples (or hnsw.max_scan_tuples)
...
$node->safe_psql("postgres", "CREATE TABLE tst_sparsevec (i int4, v sparsevec(3));");
$node->safe_psql("postgres", "INSERT INTO tst_sparsevec SELECT i, v::sparsevec(3) FROM tst;");
...
my $vec = "[" . join(",", @r) . "]";
my $sparse = "{" . join(",", map { ($_ + 1) . ":" . $r[$_] } 0 .. 2) . "}/3";
...
my @opclasses = (
	...
	["tst", "vector_l1_ops", "<+>", $vec],
	["tst_halfvec", "halfvec_l2_ops", "<->", $vec],
	["tst_sparsevec", "sparsevec_l2_ops", "<->", $sparse],
	["tst_bit", "bit_hamming_ops", "<~>", $bits],
	["tst_bit", "bit_jaccard_ops", "<%>", $bits]
);
...
for my $opclass (@opclasses)
{
	...
	$node->safe_psql("postgres", "CREATE INDEX idx ON $table USING hnsw (v $ops);");
...
$node->safe_psql("postgres", "CREATE INDEX ON ptst USING hnsw (v vector_l2_ops);");
$node->safe_psql("postgres",
...
test_exact("join", $cost,
	"SELECT tst.i, tst.$dist FROM tst INNER JOIN (SELECT generate_series(0, 9) AS c) g ON g.c = tst.c ORDER BY tst.$dist;");
test_exact("materialized cte", $cost,
	"WITH n AS MATERIALIZED (SELECT i, $dist AS d FROM tst ORDER BY $dist) SELECT * FROM n;");
test_exact("partitioned table", $cost, "SELECT i, $dist FROM ptst ORDER BY $dist;");
test_exact("iterative scan", "$cost SET hnsw.iterative_scan = relaxed_order; SET hnsw.max_scan_tuples = 1000;",
...
test_count("create table as: all rows", $cost,
	"CREATE TABLE ctas AS SELECT i FROM tst ORDER BY $dist; SELECT COUNT(*) FROM ctas; DROP TABLE ctas;", $rows);

# Test partial index
$node->safe_psql("postgres", "CREATE INDEX partial_idx ON tst USING hnsw (v vector_l2_ops) WHERE (c = 1);");
...
my @index_queries = (
	...
	["iterative scan with limit", "SET hnsw.iterative_scan = relaxed_order;", "SELECT i FROM tst ORDER BY $dist LIMIT 10;"],
